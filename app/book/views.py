...
from .serializers import SeatAllotSerializer, SeatAllotResponseSerializer, VacateSeatSerializer
...
class occupySeat(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            for key, values in seats.items():
                if seats[key] is not None:
                    if seats[key]['ticket'] == ticket:
                        return Response(data={"Message": "A ticket with this UUID has already been booked!"})
                    else:
                        pass
                else:
            ...
            print(seats)
            response_serializer = SeatAllotResponseSerializer(data=data)
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if settings.MAX_OCCUPANCY >= seat_number:
            if seats[seat_number]:
                ...
                print(seats)
                return Response(data={"Message": "The provided seat number has been vacated!"}, status=status.HTTP_204_NO_CONTENT)
            else:
                print(seats)
                return Response(data={"Message": "The provided seat number is vacant already!"}, status=status.HTTP_404_NOT_FOUND)
        print(seats)
        return Response(data={"Message": "Seat number doesn't exist!"}, status=status.HTTP_404_NOT_FOUND)